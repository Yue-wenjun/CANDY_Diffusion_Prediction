...
class CANDY(nn.Module):
    def __init__(self, batch_size, in_channel, hidden_channel, out_channel, input_size, hidden_size):
        ...
        self.fc0 = nn.Linear(in_channel, hidden_channel)
    ...
    def forward(self, x):
        origin = x
        x = x.view(self.in_channel, self.hidden_size, self.input_size)
        x = x.contiguous().view(self.hidden_size, self.input_size, self.in_channel)
        x = self.fc0(x)
        x = x.view(self.hidden_channel, self.hidden_size, self.input_size)
        ...
        p_output = torch.Tensor(self.hidden_channel, self.hidden_size, self.input_size).to(x.device)
        z_output = torch.Tensor(self.hidden_channel, self.hidden_size, self.input_size).to(x.device)
        ...
        combined_output = combined_output.view(self.hidden_size, self.input_size, self.hidden_channel)
        output = self.fc1(combined_output)
        output = output.view(self.batch_size, self.out_channel, self.hidden_size, self.input_size)
